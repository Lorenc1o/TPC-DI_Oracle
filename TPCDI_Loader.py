...
class TPCDI_Loader():
  ...
  def load_target_dim_security(self):
    ...
    SELECT SS.SYMBOL,SS.ISSUE_TYPE, ST.ST_NAME, SS.NAME, SS.EX_ID, DC.SK_CompanyID, SS.SH_OUT, TO_DATE(SS.FIRST_TRADE_DATE,'YYYY-MM-DD'),
          TO_DATE(FIRST_TRADE_EXCHANGE, 'YYYY-MM-DD'), SS.DIVIDEN, 'true', {0}, TO_DATE(LPAD(SS.PTS,8),'YYYY-MM-DD'), TO_DATE('99991231','YYYY-MM-DD')
    FROM S_Security SS
  ...
  def load_market_history(self):
    ...
    INSERT INTO FactMarketHistory (SK_SecurityID, SK_CompanyID, SK_DateID, PERatio, Yield, FiftyTwoWeekHigh,
        ...
        WITH MAXCLOSE AS (
            SELECT DM_DATE, DM_S_SYMB, DM_CLOSE, MAX(DM_CLOSE) OVER (PARTITION BY DM_S_SYMB ORDER BY DM_DATE ROWS BETWEEN 365 PRECEDING AND CURRENT ROW) AS MAXC
          ...
          MAXDATE AS (
            ...
            WHERE DM.DM_DATE >= MC.DM_DATE - 365 AND DM.DM_DATE < MC.DM_DATE AND DM.DM_CLOSE = MC.MAXC
            GROUP BY DM.DM_S_SYMB, MC.DM_DATE
          ),
          MINCLOSE AS (
            SELECT DM.DM_DATE, DM.DM_S_SYMB, MIN(DM.DM_CLOSE) OVER (PARTITION BY DM.DM_S_SYMB ORDER BY DM.DM_DATE ROWS BETWEEN 365 PRECEDING AND CURRENT ROW) AS MINC
            FROM S_Daily_Market DM),
          MINDATE AS (
          SELECT DM.DM_S_SYMB AS SYMB, MIN(DM.DM_DATE) AS MIND, MAX(MC.MINC) AS MINC, MC.DM_DATE AS BASEDATE
            ...
            WHERE DM.DM_DATE >= MC.DM_DATE - 365 AND DM.DM_DATE < MC.DM_DATE AND DM.DM_CLOSE = MC.MINC
            GROUP BY DM.DM_S_SYMB, MC.DM_DATE
          ),
          EPS AS (
    ...
    with oracledb.connect(
            ...
            dsn=self.oracle_host + '/' + self.oracle_db) as connection:
      with connection.cursor() as cursor:
        cursor.execute(insert_dimmarkethistory_query)
      connection.commit()
    print('Done.')
