/* ++++++++++++++++++++++++++++++++++++++++++++++++++ *
 * +                                                + *
 * +        TPC-DI  Automated Audit_ Script          + *
 * +        Version 1.1.0                           + *
 * +                                                + *
 * ++++++++++++++++++++++++++++++++++++++++++++++++++ *
 *                                                    *
...
--  
--  Checks against the Audit_ table.  If there is a problem with the Audit_ table, then other tests are suspect...
--  
select 'Audit_ table batches' as Test, NULL as Batch, case when 
	(select count(distinct BatchID) from Audit_) = 3 and
	(select max(BatchID) from Audit_) = 3
then 'OK' else 'Not 3 batches' end as Result, 'There must be audit data for 3 batches' as Description
 FROM dual
union
select 'Audit_ table sources' as Test, NULL as Batch, case when 
...
select 'DimBroker row count', NULL, case when 
	...
	(select Value from Audit_ where DataSet = 'DimBroker' and Attribute = 'HR_BROKERS') 
then 'OK' else 'Mismatch' end, 'Actual row count matches Audit_ table'
 FROM dual
union
...
select 'DimAccount row count', 1, case when						
	...
	(select Value from Audit_ where DataSet = 'DimCustomer' and Attribute = 'C_NEW' and BatchID = 1) + 
	(select Value from Audit_ where DataSet = 'DimAccount' and Attribute = 'CA_ADDACCT' and BatchID = 1) +
	(select Value from Audit_ where DataSet = 'DimAccount' and Attribute = 'CA_CLOSEACCT' and BatchID = 1) +
	(select Value from Audit_ where DataSet = 'DimAccount' and Attribute = 'CA_UPDACCT' and BatchID = 1) +
	(select Value from Audit_ where DataSet = 'DimCustomer' and Attribute = 'C_UPDCUST' and BatchID = 1) +
	(select Value from Audit_ where DataSet = 'DimCustomer' and Attribute = 'C_INACT' and BatchID = 1) -
	(select Value from Audit_ where DataSet = 'DimCustomer' and Attribute = 'C_ID_HIST' and BatchID = 1) -
	(select Value from Audit_ where DataSet = 'DimAccount' and Attribute = 'CA_ID_HIST' and BatchID = 1)
then 'OK' else 'Too few rows' end, 'Actual row count matches or exceeds Audit_ table minimum'
 FROM dual
union
select 'DimAccount row count', BatchID, Result, 'Actual row count matches or exceeds Audit_ table minimum' from (
	select distinct BatchID, (
		case when 
			(select count(*) from DimAccount where BatchID = a.BatchID) >=
			(select Value from Audit_ where DataSet = 'DimAccount' and Attribute = 'CA_ADDACCT' and BatchID = a.BatchID) +
			(select Value from Audit_ where DataSet = 'DimAccount' and Attribute = 'CA_CLOSEACCT' and BatchID = a.BatchID) +
			(select Value from Audit_ where DataSet = 'DimAccount' and Attribute = 'CA_UPDACCT' and BatchID = a.BatchID) -
			(select Value from Audit_ where DataSet = 'DimAccount' and Attribute = 'CA_ID_HIST' and BatchID = a.BatchID)
...
then 'OK' else 'Mismatch' end, 'BatchID values must match Audit_ table'
 FROM dual
-- union
...
-- 				EffectiveDate < (select Date from Audit_ where DataSet = 'Batch' and Attribute = 'FirstDay' and BatchID = a.BatchID) or
-- 				EffectiveDate > (select Date from Audit_ where DataSet = 'Batch' and Attribute = 'LastDay' and BatchID = a.BatchID) )
-- 		) = 0 
-- 		then 'OK' else 'Data out of range - see ticket #71' end
-- 	) as Result
-- 	from Audit_ a where BatchID in (1, 2, 3)
-- ) o
union
...
--  
union
select 'DimCustomer row count', BatchID, Result, 'Actual row count matches or exceeds Audit_ table minimum' from (
	select distinct BatchID, case when
		(select count(*) from DimCustomer where BatchID = a.BatchID) >=
		(select Value from Audit_ where DataSet = 'DimCustomer' and Attribute = 'C_NEW' and BatchID = a.BatchID) + 
		(select Value from Audit_ where DataSet = 'DimCustomer' and Attribute = 'C_INACT' and BatchID = a.BatchID) +
		(select Value from Audit_ where DataSet = 'DimCustomer' and Attribute = 'C_UPDCUST' and BatchID = a.BatchID) -
		(select Value from Audit_ where DataSet = 'DimCustomer' and Attribute = 'C_ID_HIST' and BatchID = a.BatchID)
...
then 'OK' else 'Bad value' end, 'All Status values are valid'
 FROM dual
union
select 'DimCustomer inactive customers', BatchID, Result, 'Inactive customer count matches Audit_ table' from (
	select distinct BatchID, case when
		...
		(select sum(Value) from Audit_ where DataSet = 'DimCustomer' and BatchID <= a.BatchID and Attribute = 'C_INACT')		
...
select 'DimCustomer age range alerts', BatchID, Result, 'Count of age range alerts matches audit table' from (
	select distinct BatchID, case when
		...
		(select Value from Audit_ where DataSet = 'DimCustomer' and BatchID = a.BatchID and Attribute = 'C_DOB_TO') +
		(select Value from Audit_ where DataSet = 'DimCustomer' and BatchID = a.BatchID and Attribute = 'C_DOB_TY')
	then 'OK' else 'Mismatch' end as Result
	from Audit_ a where BatchID in (1, 2, 3)
) o
union
select 'DimCustomer customer tier alerts', BatchID, Result, 'Count of customer tier alerts matches audit table' from (
	select distinct BatchID, case when
		...
		(select Value from Audit_ where DataSet = 'DimCustomer' and BatchID = a.BatchID and Attribute = 'C_TIER_INV')
	then 'OK' else 'Mismatch' end as Result
	from Audit_ a where BatchID in (1, 2, 3)
) o
union                     
...
--  
union
select 'DimSecurity row count', BatchID, Result, 'Actual row count matches or exceeds Audit_ table minimum' from (
	select distinct BatchID, case when
		...
		(select sum(Value) from Audit_ where DataSet = 'DimSecurity' and Attribute = 'FW_SEC' and BatchID <= a.BatchID)
	then 'OK' else 'Too few rows' end as Result
	from Audit_ a where BatchID in (1)
) o
union
...
--  
union
select 'DimCompany row count', BatchID, Result, 'Actual row count matches or exceeds Audit_ table minimum' from (
	select distinct BatchID, case when
		...
		(select sum(Value) from Audit_ where DataSet = 'DimCompany' and Attribute = 'FW_CMP' and BatchID <= a.BatchID)
	then 'OK' else 'Too few rows' end as Result
	from Audit_ a where BatchID in (1, 2, 3)
) o
union
	...
	select distinct BatchID, (
		case when (
			...
			where p.BatchID = a.BatchID and (
				...
				(select Date from Audit_ where DataSet = 'Batch' and Attribute = 'FirstDay' and BatchID = a.BatchID)
			)
		) = 0
...
--
union
select 'FactWatches row count', BatchID, Result, 'Actual row count matches Audit_ table' from (
	select distinct BatchID, (
		case when 
			...
			(select Value from Audit_ where DataSet = 'FactWatches' and Attribute = 'WH_ACTIVE' and BatchID = a.BatchID)
		then 'OK' else 'Mismatch' end
	) as Result
	from Audit_ a where BatchID in (1, 2, 3)
) o
...
then 'OK' else 'Mismatch' end, 'BatchID values must match Audit_ table'
 FROM dual
union
select 'FactWatches active watches', BatchID, Result, 'Actual total matches Audit_ table' from (
	select distinct BatchID, case when
		...
		(select sum(Value) from Audit_ where DataSet = 'FactWatches' and Attribute = 'WH_RECORDS' and BatchID <= a.BatchID)
	then 'OK' else 'Mismatch' end as Result
	from Audit_ a where BatchID in (1, 2, 3)
) o
union
		...
		case when (
			...
			where w.BatchID = a.BatchID and (
				w.SK_DateID_DateRemoved is null and (
					...
					(select Date from Audit_ where DataSet = 'Batch' and Attribute = 'LastDay' and BatchID = a.BatchID)
					or
					(select DateValue from DimDate where SK_DateID = w.SK_DateID_DatePlaced) < 
					(select Date from Audit_ where DataSet = 'Batch' and Attribute = 'FirstDay' and BatchID = a.BatchID)
				) or 
...
-- 
union
select 'DimTrade row count', BatchID, Result, 'Actual total matches Audit_ table' from (
	select distinct BatchID, case when
		...
		(select sum(Value) from Audit_ where DataSet = 'DimTrade' and Attribute = 'T_NEW' and BatchID <= a.BatchID)
	then 'OK' else 'Mismatch' end as Result
	from Audit_ a where BatchID in (1, 2, 3)
) o
union 
select 'DimTrade canceled trades', NULL, case when 
		...
		(select sum(Value) from Audit_ where DataSet = 'DimTrade' and Attribute = 'T_CanceledTrades')
then 'OK' else 'Mismatch' end, 'Actual row counts matches Audit_ table'
 FROM dual
union 
select 'DimTrade commission alerts', NULL, case when 
		...
		(select sum(Value) from Audit_ where DataSet = 'DimTrade' and Attribute = 'T_InvalidCommision')
then 'OK' else 'Mismatch' end, 'Actual row counts matches Audit_ table'
 FROM dual
union 
select 'DimTrade charge alerts', NULL, case when 
		...
		(select sum(Value) from Audit_ where DataSet = 'DimTrade' and Attribute = 'T_InvalidCharge')
then 'OK' else 'Mismatch' end, 'Actual row counts matches Audit_ table'
 FROM dual
union
			...
			where w.BatchID = a.BatchID and (
				w.SK_CloseDateID is null and (
					...
					(case when w.Type like 'Limit%'  /* Limit trades can have create dates earlier than the current Batch date, but not earlier than Batch1's first date */
                                              then (select Date from Audit_ where DataSet = 'Batch' and Attribute = 'FirstDay' and BatchID = 1) 
                                              else (select Date from Audit_ where DataSet = 'Batch' and Attribute = 'FirstDay' and BatchID = a.BatchID) end)
				) or 
...
select 'Financial row count', NULL, case when 
		...
		(select sum(Value) from Audit_ where DataSet = 'Financial' and Attribute = 'FW_FIN')
then 'OK' else 'Mismatch' end, 'Actual row count matches Audit_ table'
 FROM dual
union
...
select 'Financial FI_YEAR', NULL, case when (
	(select count(*) from Financial where FI_YEAR < year((select Date from Audit_ where DataSet = 'Batch' and BatchID = 1 and Attribute = 'FirstDay'))) + 
	(select count(*) from Financial where FI_YEAR > year((select Date from Audit_ where DataSet = 'Batch' and BatchID = 1 and Attribute = 'LastDay')))
) = 0
...
--  
union
select 'FactMarketHistory row count', BatchID, Result, 'Actual row count matches Audit_ table' from (
	select distinct BatchID, (
		case when 
			...
			(select Value from Audit_ where DataSet = 'FactMarketHistory' and Attribute = 'DM_RECORDS' and BatchID = a.BatchID)
		then 'OK' else 'Mismatch' end
	) as Result
	from Audit_ a where BatchID in (1, 2, 3)
) o
...
select 'FactMarketHistory SK_DateID', BatchID, Result, 'All dates are within batch date range' from (
	select distinct BatchID, (
		case when (
			(select count(*) from FactMarketHistory m where m.BatchID = a.BatchID and (select DateValue from DimDate where SK_DateID = m.SK_DateID) < (select Date-1 day from Audit_ where DataSet = 'Batch' and BatchID = a.BatchID and Attribute = 'FirstDay')) + 
			(select count(*) from FactMarketHistory m where m.BatchID = a.BatchID and (select DateValue from DimDate where SK_DateID = m.SK_DateID) >= (select Date from Audit_ where DataSet = 'Batch' and BatchID = a.BatchID and Attribute = 'LastDay'))
		) = 0
...
--  
union
select 'FactHoldings row count', BatchID, Result, 'Actual row count matches Audit_ table' from (
	select distinct BatchID, (
		case when 
			...
			(select Value from Audit_ where DataSet = 'FactHoldings' and Attribute = 'HH_RECORDS' and BatchID = a.BatchID)
		then 'OK' else 'Mismatch' end
	) as Result
	from Audit_ a where BatchID in (1, 2, 3)
) o 
...
select 'FactHoldings SK_DateID', BatchID, Result, 'All dates are within batch date range' from (
	select distinct BatchID, (
		case when (
			(select count(*) from FactHoldings m where BatchID = a.BatchID and (select DateValue from DimDate where SK_DateID = m.SK_DateID) < (select Date from Audit_ where DataSet = 'Batch' and BatchID = a.BatchID and Attribute = 'FirstDay')) + 
			(select count(*) from FactHoldings m where BatchID = a.BatchID and (select DateValue from DimDate where SK_DateID = m.SK_DateID) > (select Date from Audit_ where DataSet = 'Batch' and BatchID = a.BatchID and Attribute = 'LastDay'))
		) = 0
		then 'OK' else 'Bad Date' end
	) as Result
	from Audit_ a where BatchID in (1, 2, 3)
...
select 'FactCashBalances SK_DateID', BatchID, Result, 'All dates are within batch date range' from (
	select distinct BatchID, (
		case when (
			(select count(*) from FactCashBalances m where BatchID = a.BatchID and (select DateValue from DimDate where SK_DateID = m.SK_DateID) < (select Date from Audit_ where DataSet = 'Batch' and BatchID = a.BatchID and Attribute = 'FirstDay')) + 
			(select count(*) from FactCashBalances m where BatchID = a.BatchID and (select DateValue from DimDate where SK_DateID = m.SK_DateID) > (select Date from Audit_ where DataSet = 'Batch' and BatchID = a.BatchID and Attribute = 'LastDay'))
		) = 0
		then 'OK' else 'Bad Date' end
	) as Result
	from Audit_ a where BatchID in (1, 2, 3)
